(function () {
    ...
    function init() {
        if ($(window).width() > 998) {
            ...
            $('.listings-grid-map__listings-wrap').height(listingsBlockHeight);
            $('.listings-grid-map__map').height(listingsBlockHeight);
        } else {
            $('.listings-grid-map__listings-wrap').css('height', 'auto');
        }
    }

    noUiSlider.create($('.listings-grid-map__filter-price-range-slider').get(0), {
        connect: true,
        behaviour: 'tap',
        start: [50000, 270000],
    ...
    var map = L.map('listings-grid-map-map').setView([51.505, -0.09], 13);
